"""API-level coverage for education recurring contributions: CRUD, ownership scoping,
and that an active recurring contribution (monthly or yearly) is automatically folded
into a growth simulation - the education-module equivalent of
test_retirement_recurring_contributions.py's coverage.
"""
...
async def _create_account(client, auth_headers, payload=FIVE_TWENTY_NINE_PAYLOAD):
    response = await client.post("/api/v1/education-accounts", json=payload, headers=auth_headers)
...
async def test_create_recurring_contribution_requires_authentication(client, auth_headers):
    ...
    response = await client.post(
        f"/api/v1/education-accounts/{account_id}/recurring-contributions",
        json={"amount": "200", "frequency": "monthly", "start_date": "2026-01-01"},
    )
...
async def test_create_and_list_recurring_contribution(client, auth_headers):
    ...
    list_response = await client.get(
        f"/api/v1/education-accounts/{account_id}/recurring-contributions", headers=auth_headers
    )
...
async def test_update_and_delete_recurring_contribution(client, auth_headers):
    ...
    update_response = await client.put(
        f"/api/v1/education-accounts/recurring-contributions/{contribution_id}",
        json={"amount": "250", "is_active": False},
        headers=auth_headers,
    )
    assert update_response.status_code == 200
    assert float(update_response.json()["amount"]) == 250.00
...
async def test_recurring_contributions_are_scoped_to_their_owner(client):
    ...
    list_response = await client.get(
        f"/api/v1/education-accounts/{account_id}/recurring-contributions", headers=b_headers
    )
...
async def test_simulation_includes_monthly_recurring_contribution_by_default(client, auth_headers):
    ...
    assert float(response.json()["total_contributions"]) == 2400.00  # 200 * 12
...
async def test_simulation_folds_yearly_contribution_into_monthly_equivalent(client, auth_headers):
    ...
    assert float(response.json()["total_contributions"]) == 1200.00
...
async def test_simulation_excludes_recurring_when_include_recurring_is_false(client, auth_headers):
    ...
    assert response.status_code == 200
    assert float(response.json()["total_contributions"]) == 0.0
